...
def buildMode(inputNodes, outputNodes, params_fileName='params.mat', structure_fileName='structure.txt'):
    ...
    return theano.function([x], [layeri.y_pred[0], layeri.output[0]])
...
if __name__ == '__main__':
    #'VegetationReducedData1500_10.mat', 'U_PaviaReducedData1960_10.mat'
    test_set_x, test_set_y = loadData('VegetationReducedData1500_10.mat', -1)
    ...
    for i in xrange(outputNodes):
        Detect_models.append(buildMode(inputNodes, outputNodes, 'params%d.mat' %(i)))
    print 'start'
    ...
    for i in xrange(outputNodes):
        for j in xrange(test_set_x.shape[0]):
            if test_set_y[j] == i:
                label = -1
                prob = 0.0
                for k in xrange(outputNodes):
                    [labelk, probk] = Detect_models[k](test_set_x[j:j+1])
                    if (probk[0] > prob):
                        prob = probk[0]
                        label = k
